...
public class BiomeTriassicChinaTrees extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:triassic_china_trees")
	public static final BiomeGenCustom biome = null;

	public BiomeTriassicChinaTrees(ElementsLepidodendronMod instance) {
		super(instance, 1589);
	}
	...
	static class BiomeGenCustom extends BiomeTriassic {
		public BiomeGenCustom() {
			//was height 0.001
			super(new BiomeProperties("Triassic Cathay Islands").setBaseHeight(0.0075F).setHeightVariation(0.0015F).setTemperature(0.8F).setRainfall(0.9F));
			setRegistryName("lepidodendron:triassic_china_trees");
		...
		protected static final WorldGenElatocladusTree ELACTOCLADUS_TREE = new WorldGenElatocladusTree(false);
		...
		public void decorate(World worldIn, Random rand, BlockPos pos) {
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 8; ++i) {
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					PLANT_GENERATOR.generate(BlockBaiera.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 60, 80);
				}
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 11; ++i) {
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					PLANT_GENERATOR.generate(BlockPtilozamites.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 60, 80);
				}
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 5; ++i) {
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					PLANT_GENERATOR.generate(BlockVoltzia.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 60, 80);
				}
		...
		public EnumBiomeTypeTriassic getBiomeType() {
			return EnumBiomeTypeTriassic.China;
		}
