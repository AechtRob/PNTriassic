...
public class BiomeTriassicCreekHorsetails extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:triassic_creek_horsetails")
	public static final BiomeGenCustom biome = null;
	public BiomeTriassicCreekHorsetails(ElementsLepidodendronMod instance) {
		super(instance, 1589);
	}
	...
	static class BiomeGenCustom extends BiomeTriassic {
		public BiomeGenCustom() {
			super(new BiomeProperties("Triassic Equisetites Lea Creek").setTemperature(0.75F).setRainfall(0.8F).setBaseHeight(-0.525F).setHeightVariation(0.0F).setWaterColor(-5317633));
			setRegistryName("lepidodendron:triassic_creek_horsetails");
		...
		public void decorate(World worldIn, Random rand, BlockPos pos)
		{
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 4; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					LEAFBLOCK_GENERATOR.generate((BlockBush) BlockElatocladusSapling.block, BlockElatocladusLeaves.block.getDefaultState(), BlockElatocladusLog.block.getDefaultState().withProperty(BlockElatocladusLog.BlockCustom.FACING, EnumFacing.NORTH), worldIn, rand, pos.add(j, l, k), 0, 75);
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 1; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					ANOMOZAMITES_GENERATOR.generate(worldIn, rand, pos.add(j, l, k), 60, 255);
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 32; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					PLANT_GENERATOR.generate(BlockMopHorsetail.block.getDefaultState(), worldIn, rand, pos.add(j, l, k));
				}
