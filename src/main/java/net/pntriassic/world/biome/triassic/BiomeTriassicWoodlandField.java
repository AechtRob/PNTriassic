...
public class BiomeTriassicWoodlandField extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:triassic_woodland_field")
	public static final BiomeGenCustom biome = null;
	public BiomeTriassicWoodlandField(ElementsLepidodendronMod instance) {
		super(instance, 1589);
	}
	...
	static class BiomeGenCustom extends BiomeTriassic {
		...
		protected static final WorldGenSilverTreefernTree TREEFERN_TREE = new WorldGenSilverTreefernTree(false);
		...
		protected static final WorldGenTreeLog PAGIOPHYLLUM_LOG_GENERATOR = new WorldGenTreeLog(BlockPagiophyllumLog.block);
		...
		public void decorate(World worldIn, Random rand, BlockPos pos)
		{
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
			{
				...
				if (rand.nextInt(3) == 0) {
					for (int j = 0; j < i; ++j) {
						...
						if (Math.random() > 0.5) {
							PAGIOPHYLLUM_LOG_GENERATOR.generate(worldIn, rand, blockpos);
						}
					}
				}
			}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 64; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					PLANT_GENERATOR.generate(BlockMarattia.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 0, 255, true);
				}
